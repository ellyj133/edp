            ...
            switch ($action) {
                ...
                case 'save_banner':
                    ...
                    if ($id) {
                        ...
                        $stmt = $pdo->prepare("
                            UPDATE banners 
                            SET title = ?, image_url = ?, link_url = ?, alt_text = ?, position = ?, 
                                sort_order = ?, status = ?, starts_at = ?, expires_at = ?
                            WHERE id = ?
                        ");
                        $stmt->execute([
                            $title, $image_url, $link_url, $alt_text, $position, 
                        ...
                        $stmt = $pdo->prepare("
                            INSERT INTO banners 
                            (title, image_url, link_url, alt_text, position, sort_order, status, 
                             starts_at, expires_at, created_by)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                        ");
                        $stmt->execute([
                            $title, $image_url, $link_url, $alt_text, $position, $sort_order, 
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                <td>
                                                    ...
                                                    <br><small class="text-muted"><?= htmlspecialchars($banner['image_url']) ?></small>
                                                    <?php endif; ?>
                                                </td>
