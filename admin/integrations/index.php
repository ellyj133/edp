    ...
    try {
        switch ($_POST['action']) {
            case 'create_api_key':
                ...
                $stmt = $db->prepare("
                    INSERT INTO api_keys (name, description, api_key, api_secret, scopes, rate_limit, created_by, created_at) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, NOW())
                ");
                $stmt->execute([$name, $description, $api_key, hash('sha256', $api_secret), json_encode($scopes), $rate_limit, $_SESSION['user_id']]);
            ...
            case 'create_webhook':
                ...
                $stmt = $db->prepare("
                    INSERT INTO webhook_subscriptions (name, url, events, secret, created_by, created_at) 
                    VALUES (?, ?, ?, ?, ?, NOW())
                ");
                $stmt->execute([$name, $url, json_encode($events), $secret, $_SESSION['user_id']]);
                
                $success = "Webhook created successfully! Secret: " . $secret;
                break;
                
            case 'test_webhook':
                $id = intval($_POST['webhook_id']);
            ...
            case 'update_integration':
                ...
                $stmt = $db->prepare("
                    INSERT INTO integrations (name, config, enabled, updated_by, updated_at) 
                    VALUES (?, ?, ?, ?, NOW())
                    ON DUPLICATE KEY UPDATE 
                    config = VALUES(config), enabled = VALUES(enabled), updated_by = VALUES(updated_by), updated_at = VALUES(updated_at)
                ");
                $stmt->execute([$integration, json_encode($config), $enabled, $_SESSION['user_id']]);
...
if ($database_available) {
    try {
        ...
        $stmt = $db->query("
            SELECT ws.*, u.username as created_by_name,
                   (SELECT COUNT(*) FROM webhook_deliveries WHERE webhook_id = ws.id AND success = 1 AND created_at > DATE_SUB(NOW(), INTERVAL 24 HOUR)) as successful_24h,
                   (SELECT COUNT(*) FROM webhook_deliveries WHERE webhook_id = ws.id AND success = 0 AND created_at > DATE_SUB(NOW(), INTERVAL 24 HOUR)) as failed_24h
    ...
    $api_keys = [
        [
            'id' => 1,
            'name' => 'Demo API Key',
            'description' => 'This is demo data for development',
            'api_key' => 'epd_demo_key_12345',
            'scopes' => json_encode(['read:products', 'read:orders']),
            'status' => 'active',
            'usage_24h' => 42,
    ...
    $webhooks = [
        [
            ...
            'events' => json_encode(['order.created', 'payment.completed']),
            'status' => 'active',
            'successful_24h' => 12,
            'failed_24h' => 1,
            'created_at' => '2024-01-15 10:00:00'
        ...
        <div class="row">
            ...
            <div class="col-md-6 col-lg-4 mb-4">
                <div class="webhook-card card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h6 class="mb-0"><?php echo htmlspecialchars($webhook['name']); ?></h6>
                        <span class="status-indicator status-<?php echo $webhook['status']; ?>"></span>
                    </div>
                    <div class="card-body">
                        <div class="mb-2">
                        ...
                        <div class="row">
                            <div class="col-6 text-center">
                                <div class="text-success h5 mb-0"><?php echo number_format($webhook['successful_24h']); ?></div>
                                <small class="text-muted">Success (24h)</small>
                            </div>
                            <div class="col-6 text-center">
                                <div class="text-danger h5 mb-0"><?php echo number_format($webhook['failed_24h']); ?></div>
